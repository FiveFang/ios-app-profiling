...
def monitor_battery_hybrid(device_id, duration_minutes=30, interval_seconds=30, app_bundle_id=None):
    ...
    if instruments_available:
        # Use Time Profiler and CPU Profiler templates for advanced CPU analysis
        profiler_templates = ["Time Profiler", "CPU Profiler"]
        for template in profiler_templates:
            instruments_cmd = [
                ...
                "--output", trace_file.replace(".trace", f"_{template.replace(' ', '_')}.trace")
            ]
            try:
                console.print(f"[dim]📊 Starting Instruments {template} profiling (WiFi connection)...[/dim]")
                ...
                console.print(f"[dim]✅ Instruments {template} profiling started successfully[/dim]")
            except Exception as e:
                console.print(f"[yellow]⚠️  Failed to start Instruments {template}: {e}[/yellow]")
                instruments_process = None
    else:
        console.print("[dim]📝 Skipping Instruments profiling - using WiFi connection only[/dim]")
...
def launch_app(device_id, bundle_id):
    ...
    try:
        # Try idevicedebug first (works better with network connections)
        ...
        if process.poll() is None:
            ...
            if stderr:
                console.print(f"[yellow]idevicedebug error: {stderr.decode().strip()}[/yellow]")
    except Exception as e:
...
def parse_instruments_trace(trace_file, app_bundle_id=None):
    ...
    try:
        ...
        # Export table of contents first
        ...
        if result.returncode != 0:
            console.print(f"[yellow]⚠️  Could not export table of contents: {result.stderr.strip()}[/yellow]")
            console.print(f"[yellow]⚠️  Trace parsing failed - using fallback estimation[/yellow]")
            return None
        ...
        try:
            ...
            # Extract basic info from the trace
            power_data = {
                "total_energy_cost": 1.0,  # Fallback estimate
                "app_energy_cost": 0.5,    # App portion
                "cpu_energy_cost": 0.3,
                "gpu_energy_cost": 0.05,
                "network_energy_cost": 0.05,
                "display_energy_cost": 0.1,
                "confidence": "medium"
            }
...
def compare_test(device, app, duration, interval):
    ...
    device_name = target_device.get("deviceProperties", {}).get("name", "")
    
    console.print(f"[green]🎯 Target Device: {device_name} ({device_id})[/green]")
    console.print(f"[cyan]🔬 Running comparative battery analysis...[/cyan]")
    
    # Run baseline and app tests
    console.print(f"\n[cyan]📊 Phase 1: Baseline Test (No App)[/cyan]")
    baseline_results = monitor_battery_hybrid(device_id, duration, interval)
    
    time.sleep(5)  # Brief pause between tests
    
    console.print(f"\n[cyan]📊 Phase 2: App Test ({app})[/cyan]") 
