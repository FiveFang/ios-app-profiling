...
8. [Device Profiling Parser](#device-profiling-parser)
9. [Real-time Monitoring](#real-time-monitoring)
10. [Energy Metrics Breakdown](#energy-metrics-breakdown)
...
## Device Profiling Parser

### Overview
The Device Profiling Parser (`device_profiling_parser.py`) is a standalone utility that analyzes profiling files created directly on iOS devices. These `.aar` files contain energy consumption data collected using Apple's on-device profiling tools.

### Supported File Formats
- **`.aar` files**: Apple Archive format containing profiling data
- **Timing extraction**: Automatic session timing from filenames and metadata
- **Energy estimation**: Device-specific calculations with app targeting
...
│  │                 │  │                 │  │              │ │
│  │ • .aar Scanner  │  │ • xctrace       │  │ • Energy     │ │
│  │ • Metadata      │  │   Export        │  │   Estimation │ │
│  │   Extraction    │  │ • Binary        │  │ • Timing     │ │
│  │ • Timing        │  │   Analysis      │  │   Analysis   │ │
│  │   Detection     │  │ • Metadata      │  │ • App        │ │
│  └─────────────────┘  │   Parsing       │  │   Targeting  │ │
│                       └─────────────────┘  └──────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
#### 1. Instruments Export Method
```python
def parse_with_instruments_export(aar_file_path):
    """Export .aar file using xctrace and parse resulting XML"""
    export_cmd = [
        "xcrun", "xctrace", "export", 
        "--input", aar_file_path,
        "--toc", "--output", "temp_toc.xml"
    ]
    # Parse XML structure for timing and energy data
```

#### 2. Binary Analysis Method
```python
def parse_with_binary_analysis(aar_file_path):
    """Direct binary analysis of .aar file structure"""
    with open(aar_file_path, 'rb') as f:
        # Read file header and extract metadata
        header = f.read(1024)
        # Parse binary structure for timing information
```

#### 3. Metadata Extraction
```python
def extract_timing_from_filename(filename):
    """Extract session timing from filename patterns"""
    # PowerProfiler_25-10-04_185707_to_25-10-04_185729.aar
    # Extracts: start=18:57:07, end=18:57:29, duration=22 seconds
```
...
# Output: Lists all .aar files with basic information
```

#### Parse Single File
```bash
python device_profiling_parser.py parse-aar PowerProfiler_file.aar
# Output: JSON with timing, energy estimates, and metadata
```

#### Detailed Analysis
```bash
python device_profiling_parser.py analyze PowerProfiler_file.aar --detailed
# Output: Comprehensive analysis with multiple parsing methods
```

### Energy Estimation Algorithm
```python
def estimate_energy_consumption(duration_seconds, device_model="iPhone"):
    """Estimate energy consumption with percentage calculations"""
    
    # Get device-specific battery capacity
    device_capacity = get_device_battery_capacity()  # e.g., 3582 mAh for iPhone 16 Pro
    
    # Base consumption rates (mAh per hour)
    base_rates = {
        "iPhone": {"idle": 50, "active": 200, "intensive": 400},
        "iPad": {"idle": 80, "active": 300, "intensive": 600}
    }
    
    # Calculate total consumption
    duration_hours = duration_seconds / 3600
    total_consumption = base_rates[device_model]["active"] * duration_hours
    
    # App-specific estimation (typically 15-30% of total)
    app_consumption = total_consumption * 0.2
    
    # Calculate percentages based on actual device capacity
    total_percentage = (total_consumption / device_capacity) * 100
    app_percentage = (app_consumption / device_capacity) * 100
    
    return {
        "total_mah": round(total_consumption, 3),
        "app_mah": round(app_consumption, 3),
        "device_capacity_mah": device_capacity,
        "total_percentage": round(total_percentage, 3),
        "app_percentage": round(app_percentage, 3),
...
{
  "file_info": {
    "filename": "PowerProfiler_25-10-04_185707_to_25-10-04_185729.aar",
    "size_kb": 914.7,
  ...
  "energy_estimates": {
    "total_consumption_mah": 0.73,
    "app_consumption_mah": 0.29,
    "device_capacity_mah": 3582,
    "total_consumption_percentage": 0.020,
    "app_consumption_percentage": 0.008,
    "methodology": "duration_based_estimation"
  }
}
```

### Integration with Main Tool
While the device profiling parser operates independently, it can complement the main `instruments_tester.py` utility:

1. **On-device profiling** → Create .aar files on iOS device
2. **File transfer** → Move .aar files to Mac for analysis  
3. **Parser analysis** → Extract timing and energy estimates
4. **Comparison** → Compare with live Instruments profiling results
...
### Device Profiling Parser Issues
```bash
# .aar file not found
python device_profiling_parser.py scan  # List available files

# Parsing failures
python device_profiling_parser.py analyze file.aar --detailed  # Try all methods

# Empty results
ls -la *.aar  # Check file size (should be > 100 KB)
file *.aar    # Verify file type
```

**Common Device Profiling Issues:**
- **Empty .aar files**: Profiling session too short (< 10 seconds)
- **Parse failures**: File corruption during transfer from device
- **No timing data**: Filename doesn't match expected pattern
- **Zero energy estimates**: Duration too short for meaningful calculation
...
# Debug device profiling parser
python device_profiling_parser.py analyze file.aar --detailed --verbose
```
...
- **Device Profiling Parser**: Added standalone parser for .aar files created on iOS devices
- **Enhanced WiFi Support**: Fixed device detection for WiFi-connected iOS devices
- **Battery Capacity Detection**: Automatic device-specific battery capacity detection via `devicectl`  
- **Percentage Reporting**: Real battery consumption percentages based on actual device capacity
- **CLI Interface**: Complete CLI interface for device profiling parser with scan/parse/analyze commands
- **Comprehensive Documentation**: Added detailed technical documentation and usage guides

### Version 1.0 - October 2025
- **Initial Release**: Core Instruments integration with BatteryUserTemplate
- **Hybrid Monitoring**: Real-time battery tracking combined with Instruments profiling
- **App-Specific Testing**: Process attachment and targeted energy analysis
- **Validation Framework**: Cross-validation with confidence levels and benchmarking
- **Advanced Export**: JSON reports with trace files and comprehensive energy breakdowns
